    ...
    return (
        <div className="prose prose-sm max-w-none text-on-surface-variant">
            {lines.map((line, index) => {
                if (line.startsWith('## ')) {
                    return <h2 key={index} className="text-lg font-medium text-on-surface mt-4 mb-2">{line.substring(3)}</h2>;
                }
                if (line.startsWith('* ')) {
                    const lineContent = line.substring(2);
                    const parts = lineContent.split('**');
                    const renderedLine = parts.map((part, i) =>
                        i % 2 === 1 ? <strong key={i} className="font-medium text-on-surface">{part}</strong> : part
                    );
                    return <li key={index} className="ml-4 list-disc">{renderedLine}</li>;
                }
                ...
                return <p key={index} className="mb-2">{renderedLine}</p>;
            })}
        </div>
    );
};

const ParsedPurchaseCard: React.FC<{ purchase: Omit<PurchaseRecord, 'id'>, onSave: () => void }> = ({ purchase, onSave }) => (
    <Card className="mt-6">
...
const ShoppingListScreen: React.FC<ShoppingListScreenProps> = ({ onShowToast }) => {
    ...
    const handleGenerateList = async () => {
        ...
        try {
            ...
            onShowToast("Failed to generate list. Please try again.", 'error');
        } finally {
    ...
    const handleAddManualItem = () => {
        if (manualItem.trim()) {
            setNewListItems(prev => [...prev, { id: `manual-${Date.now()}`, name: manualItem.trim(), checked: false }]);
            setManualItem('');
        }
    };
    ...
    const handleSaveAndArchive = () => {
        ...
        setNewListTitle('');
        setPrompt('');
        onShowToast('List saved to history!', 'success');
    };
    ...
    const handleImportNFe = async () => {
        ...
        try {
            ...
            onShowToast("Failed to import invoice. The AI could not process this URL.", 'error');
        } finally {
    ...
    const handleSavePurchase = () => {
        ...
        setParsedPurchase(null);
        setNfeUrl('');
        onShowToast('Purchase record saved!', 'success');
    };

    // --- Insights Logic ---
    const handleGenerateInsights = async () => {
        if (purchaseHistory.length === 0) {
            onShowToast("You need at least one purchase record to generate insights.", "error");
            return;
        }
        ...
        try {
            ...
            onShowToast("Failed to generate insights. Please try again.", 'error');
        } finally {
    ...
    return (
        <div>
            <header className="mb-6">
                ...
                <p className="text-base text-on-surface-variant">Create lists, import purchases, and get AI-powered savings insights.</p>
            </header>
                ...
                <Tabs defaultValue="create">
                    ...
                    <TabsContent value="create">
                        <div className="p-4 md:p-6 max-w-2xl mx-auto">
                            ...
                            <p className="text-sm text-on-surface-variant mb-4">e.g., "Weekly groceries", "Ingredients for lasagna"</p>
                            <div className="flex gap-2">
                                <textarea rows={2} className="w-full p-3 bg-surface border border-outline rounded-xl flex-grow" placeholder="Describe your shopping needs..." value={prompt} onChange={(e) => setPrompt(e.target.value)} disabled={isGeneratingList} />
                                <Button onClick={handleGenerateList} disabled={isGeneratingList || !prompt} leftIcon={<SparklesIcon className="w-5 h-5"/>}>{isGeneratingList ? 'Generating...' : 'Generate'}</Button>
                            </div>
                            {isGeneratingList && <div className="flex justify-center py-8"><Spinner/></div>}
                            {newListItems.length > 0 && (
                                <div className="mt-6">
                                    <h4 className="text-xl font-medium mb-3">Your List: <span className="text-primary">{newListTitle}</span></h4>
                                    <ul className="space-y-2">
                                        {newListItems.map(item => (
                                            <li key={item.id} className="flex items-center p-2 rounded-md hover:bg-surface-variant/20">
                                                <input type="checkbox" id={item.id} checked={item.checked} onChange={() => handleToggleItem(item.id)} className="h-5 w-5 rounded border-gray-300 text-primary focus:ring-primary"/>
                                                <label htmlFor={item.id} className={`ml-3 text-on-surface ${item.checked ? 'line-through text-on-surface-variant' : ''}`}>{item.name}</label>
                                            </li>
                                        ))}
                                    </ul>
                                    <div className="mt-4 pt-4 border-t border-outline flex gap-2">
                                        <Input id="manual-item" placeholder="Add another item..." value={manualItem} onChange={e => setManualItem(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleAddManualItem()}/>
                                        <Button variant="outlined" onClick={handleAddManualItem} leftIcon={<PlusIcon className="w-5 h-5"/>}>Add</Button>
                                    </div>
                                    <div className="mt-6 text-right"><Button onClick={handleSaveAndArchive}>Save & Archive</Button></div>
                                </div>
                            )}
                        </div>
                    </TabsContent>
                    ...
                    <TabsContent value="import">
                         <div className="p-4 md:p-6 max-w-2xl mx-auto">
                            ...
                            <p className="text-sm text-on-surface-variant mb-4">Paste the public URL of a Brazilian Nota Fiscal Eletrônica to automatically log your purchase.</p>
                            <div className="flex gap-2">
                                <Input id="nfe-url" placeholder="https://sat.sef.sc.gov.br/..." value={nfeUrl} onChange={e => setNfeUrl(e.target.value)} disabled={isImporting}/>
                                <Button variant="outlined" onClick={() => onShowToast('QR Code scanning is available on the mobile app.', 'success')} leftIcon={<QrCodeIcon className="w-5 h-5"/>}>Scan</Button>
                                <Button onClick={handleImportNFe} disabled={isImporting || !nfeUrl}>{isImporting ? 'Importing...' : 'Import'}</Button>
                            </div>
                            {isImporting && <div className="flex justify-center py-8"><Spinner/></div>}
                            {parsedPurchase && <ParsedPurchaseCard purchase={parsedPurchase} onSave={handleSavePurchase} />}
                        </div>
                    </TabsContent>
                    ...
                    <TabsContent value="history">
                        <div className="p-4 md:p-6">
                            <div className="mb-6">
                                <Button onClick={handleGenerateInsights} disabled={isGeneratingInsights} leftIcon={<SparklesIcon className="w-5 h-5"/>}>
                                    {isGeneratingInsights ? 'Analyzing...' : 'Generate Savings Insights'}
                                </Button>
                            </div>
                            
                            {isGeneratingInsights && <div className="flex justify-center py-8"><Spinner/></div>}
                            {insights && (
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                <div>
                                    ...
                                    <div className="space-y-4">
                                        {purchaseHistory.length > 0 ? purchaseHistory.map(rec => (
                                            <Card key={rec.id} className="p-4 bg-surface">
                                                <details>
                                                    <summary className="font-medium text-on-surface cursor-pointer flex justify-between items-center">
                                                        <div>
                                                            <p>{rec.storeName}</p>
                                                            <p className="text-xs text-on-surface-variant">{formatDate(rec.purchaseDate)}</p>
                                                        </div>
                                                        <span className="text-sm font-medium">{formatCurrency(rec.totalAmount)}</span>
                                                    </summary>
                                                    <div className="mt-3 pt-3 border-t border-outline text-xs text-on-surface-variant">
                                                        {rec.items.map((item, i) => <p key={i}>{item.quantity}x {item.name}</p>)}
                                                    </div>
                                                </details>
                                            </Card>
                                        )) : <p className="text-center text-on-surface-variant py-8 text-sm">No purchase history yet.</p>}
                                    </div>
                                </div>
                                <div>
                                    <h3 className="text-lg font-medium text-on-surface mb-3">Past Shopping Lists</h3>
                                     <div className="space-y-4">
                                        {historicLists.length > 0 ? historicLists.map(list => (
                                            <Card key={list.id} className="p-4 bg-surface">
                                                <details>
                                                    <summary className="font-medium text-on-surface cursor-pointer flex justify-between items-center">
                                                        <div>
                                                            <p>{list.title}</p>
                                                            <p className="text-xs text-on-surface-variant">Created: {formatDate(list.createdAt)}</p>
                                                        </div>
                                                        <span className="text-sm text-on-surface-variant">{list.items.length} items</span>
                                                    </summary>
                                                    <ul className="mt-3 pt-3 border-t border-outline space-y-1">
                                                        {list.items.map(item => (
                                                            <li key={item.id} className="flex items-center text-sm">
                                                                <span className={`mr-2 ${item.checked ? 'text-secondary' : 'text-on-surface-variant'}`}>{item.checked ? '✓' : '•'}</span>
                                                                <span className={`${item.checked ? 'line-through text-on-surface-variant' : 'text-on-surface'}`}>{item.name}</span>
                                                            </li>
                                                        ))}
                                                    </ul>
                                                </details>
                                            </Card>
                                        )) : <p className="text-center text-on-surface-variant py-8 text-sm">No shopping list history yet.</p>}
                                    </div>
                                </div>
                            </div>
                        </div>
