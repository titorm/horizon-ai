...
const CategoriesScreen: React.FC = () => {
  ...
  const handleUpdateCategory = (updatedCategory: Category) => {
    setCategories(prev => prev.map(c => c.id === updatedCategory.id ? { ...c, name: updatedCategory.name, icon: updatedCategory.icon } : c));
  };
    ...
    <>
      <div className="p-4 md:p-8">
        <header className="flex justify-between items-center mb-6">
          ...
          <Button variant="outlined" leftIcon={<SettingsIcon className="w-5 h-5"/>} onClick={() => setIsModalOpen(true)}>
            Manage Categories
          </Button>
        </header>
        <main>
          <Card className="p-6">
              ...
              <p className="text-sm text-on-surface-variant mb-4">92% of transactions auto-categorized this month.</p>
