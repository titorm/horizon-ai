...
const TaxAssetRow: React.FC<{ asset: TaxAsset }> = ({ asset }) => (
    <tr className="border-b border-outline last:border-b-0 hover:bg-surface-variant/5">
        <td className="p-3 text-on-surface-variant">{asset.groupCode}.{asset.itemCode}</td>
...
const TaxIncomeRow: React.FC<{ income: TaxIncome }> = ({ income }) => (
    <tr className="border-b border-outline last:border-b-0 hover:bg-surface-variant/5">
        ...
        <td className="p-3 text-on-surface-variant text-xs">{income.sourceName}<br/>{income.sourceCnpj}</td>
    ...
    return (
        <div className="p-4 md:p-8 text-center flex flex-col items-center justify-center h-full">
            ...
            <p className="text-on-surface-variant max-w-md mb-6">
                Upgrade to Premium to get your IRPF report automatically generated from your connected accounts and investments.
            </p>
    ...
    <div className="p-4 md:p-8">
      <header className="flex justify-between items-start mb-6">
        <div>
            ...
            <p className="text-base text-on-surface-variant">Ano-calendário 2024. Prepare sua declaração com dados pré-compilados.</p>
        </div>
        <Button onClick={handleGenerateSummary} disabled={isGenerating} leftIcon={<SparklesIcon className="w-5 h-5"/>}>
      ...
      <main className="space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <Card className="p-4">
                ...
                <p className={`text-2xl font-medium ${evolucaoPatrimonial >= 0 ? 'text-secondary' : 'text-error'}`}>{formatBRL(evolucaoPatrimonial)}</p>
            </Card>
        </div>
                ...
                <div className="p-4 md:p-6">
                    <TabsContent value="bens-e-direitos">
                        <div className="flex justify-between items-center mb-4">
                            ...
                            <Button variant="outlined" leftIcon={<PlusIcon className="w-4 h-4"/>}>Adicionar Bem</Button>
                        </div>
                        <div className="overflow-x-auto border border-outline rounded-lg">
                            <table className="w-full text-left text-sm">
                                ...
                                <tbody>
                                    {assets.map(asset => <TaxAssetRow key={asset.id} asset={asset} />)}
                                </tbody>
                            </table>
                        </div>
                    </TabsContent>
                    ...
                    <TabsContent value="rendimentos-isentos">
                        <div className="flex justify-between items-center mb-4">
                            <div>
                                ...
                                <p className="text-sm text-on-surface-variant">Valores recebidos que não possuem incidência de imposto.</p>
                            </div>
                            <Button variant="outlined" leftIcon={<PlusIcon className="w-4 h-4"/>}>Adicionar Rendimento</Button>
                        </div>
                         <div className="overflow-x-auto border border-outline rounded-lg">
                             <table className="w-full text-left text-sm">
                                <thead className="bg-surface-variant/20">
                                ...
                                <tbody>
                                    {exemptIncomes.map(income => <TaxIncomeRow key={income.id} income={income} />)}
                                </tbody>
                            </table>
                        </div>
                    </TabsContent>
                        ...
                        <div className="flex justify-between items-center mb-4">
                         ...
                         <div className="overflow-x-auto border border-outline rounded-lg">
                             <table className="w-full text-left text-sm">
                                ...
                                <tbody>
                                    {exclusiveIncomes.map(income => <TaxIncomeRow key={income.id} income={income} />)}
                                </tbody>
                            </table>
                        </div>
                    </TabsContent>
