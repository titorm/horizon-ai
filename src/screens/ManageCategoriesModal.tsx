          ...
          return (
            <button
              ...
              className={`flex items-center justify-center p-2 rounded-lg border-2 transition-colors ${
                isSelected ? 'bg-primary-container border-primary' : 'bg-surface-variant/30 border-transparent hover:border-outline'
              }`}
                  ...
                  {isEditing ? (
                    <div className="space-y-4">
                      ...
                      <IconPicker 
                        ...
                        onSelectIcon={(icon) => setEditingCategory({ ...editingCategory, icon })}
                      />
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <span className="flex-grow font-medium text-on-surface">{category.name}</span>
                      {isConfirmingDelete ? (
                        <>
                          <span className="text-sm text-error">Are you sure?</span>
                          {/* FIX: Removed unsupported 'size' prop and used className for styling. */}
                          <Button variant="text" className="!text-error !px-2 !h-auto !py-1" onClick={() => handleDelete(category.id)}>Confirm</Button>
                          {/* FIX: Removed unsupported 'size' prop and used className for styling. */}
                          <Button variant="text" className="!px-2 !h-auto !py-1" onClick={() => setDeleteConfirmId(null)}>Cancel</Button>
                        </>
                      ) : (
                        <>
                          {/* FIX: Removed unsupported 'size' prop and used className for styling. */}
                          <Button variant="text" className="!px-2 !h-auto !py-1" onClick={() => handleEdit(category)}>Edit</Button>
                          {/* FIX: Removed unsupported 'size' prop and used className for styling. */}
                          <Button variant="text" className="!text-error !px-2 !h-auto !py-1" onClick={() => setDeleteConfirmId(category.id)}>Delete</Button>
                        </>
                      )}
                    </div>
                  )}
