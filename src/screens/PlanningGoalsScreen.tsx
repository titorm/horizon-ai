...
import type { FinancialGoal, SuggestedGoal } from '../../types';
import { PlusIcon, HomeIcon, PlaneIcon, LandmarkIcon, TargetIcon, EditIcon, Trash2Icon, TrendingUpIcon, SparklesIcon } from '../assets/Icons';
    ...
    <Card className="p-6 bg-primary-container border-primary">
        ...
        <div>
            ...
            <div className="flex justify-between items-center mt-2">
                <p className="text-sm text-on-primary-container">Current monthly contribution: <strong>{formatCurrency(monthlyContribution)}</strong></p>
                <Button variant="text" className="!text-primary">Adjust Plan</Button>
            </div>
        </div>
    </Card>
  );
        ...
        <Card className="p-5 flex flex-col h-full">
            ...
            <div className="mt-4 flex-grow flex flex-col justify-end">
                ...
                <div className="w-full bg-surface-variant rounded-full h-2.5">
                    <div
                        className={`${isCompleted ? 'bg-secondary' : 'bg-primary'} rounded-full h-2.5 transition-all duration-500`}
                        style={{ width: `${progress}%` }}
                    ></div>
                </div>
...
interface PlanningGoalsScreenProps {
  onShowToast: (message: string, type: 'success' | 'error') => void;
}

const PlanningGoalsScreen: React.FC<PlanningGoalsScreenProps> = ({ onShowToast }) => {
    ...
    const handleSaveGoal = () => {
        ...
        if ('id' in editingGoal) {
            setGoals(goals.map(g => g.id === (editingGoal as FinancialGoal).id ? (editingGoal as FinancialGoal) : g));
        } else {
    ...
    const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        setEditingGoal(prev => prev ? ({ ...prev, [name]: type === 'number' ? parseFloat(value) || 0 : value }) : null);
    };
    ...
    const handleGenerateGoals = async () => {
        ...
        const prompt = `
            ...
            - Meta de Aposentadoria Atual: Atingir ${formatCurrency(MOCK_RETIREMENT_GOAL.targetSavings)} aos ${MOCK_RETIREMENT_GOAL.targetAge} anos.
            - Metas existentes: ${goals.map(g => g.name).join(', ') || 'Nenhuma'}
            - Últimas transações (para entender hábitos): ${MOCK_TRANSACTIONS.slice(0, 5).map(t => `${t.description}: ${formatCurrency(t.amount)}`).join('; ')}
        ...
        try {
            ...
            onShowToast("Failed to generate suggestions. Please try again.", 'error');
        } finally {
    ...
    const handleAddSuggestedGoal = (goalToAdd: SuggestedGoal) => {
        ...
        onShowToast(`Goal "${goalToAdd.name}" added!`, 'success');
    };
    ...
    return (
        <>
            <div className="p-4 md:p-8">
                <header className="flex justify-between items-center mb-6">
                    <div>
                        ...
                        <p className="text-base text-on-surface-variant">Track your progress towards your biggest dreams.</p>
                    </div>
                    <div className="flex items-center gap-3">
                        <Button variant="outlined" leftIcon={<SparklesIcon className="w-5 h-5"/>} onClick={handleGenerateGoals} disabled={isGenerating}>
                ...
                <div className="p-6">
                    {suggestedGoals.length > 0 ? (
                        <div className="space-y-4">
                            {suggestedGoals.map((goal, index) => (
                                <Card key={index} className="p-4 bg-surface">
                                    ...
                                    <p className="text-sm text-on-surface-variant mt-1">Target: <strong>{formatCurrency(goal.targetAmount)}</strong> by <strong>{goal.targetDate}</strong></p>
                     ...
                     <div className="flex justify-end mt-6">
                        <Button variant="outlined" onClick={handleCloseModals}>Close</Button>
                    </div>
                </div>
            </Modal>
            ...
            {editingGoal && (
                ...
                >
                    <div className="p-6">
                        <form className="space-y-4" onSubmit={(e) => {e.preventDefault(); handleSaveGoal();}}>
                           <Input
                                id="name"
                                name="name"
                                label="Goal Name"
                         ...
                         <div className="flex justify-end gap-3 mt-6">
                            <Button variant="outlined" onClick={handleCloseModals}>Cancel</Button>
                ...
                >
                    <div className="p-6">
                        ...
                        <div className="flex justify-end gap-3">
                            <Button variant="outlined" onClick={handleCloseModals}>Cancel</Button>
                            <Button className="bg-error hover:opacity-90" onClick={handleConfirmDelete}>Delete</Button>
                        </div>
                    </div>
                </Modal>
            )}
...
export default PlanningGoalsScreen;