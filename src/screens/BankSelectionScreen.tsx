...
const BankListItem: React.FC<{ bank: Bank; onSelect: () => void; visible: boolean }> = ({ bank, onSelect, visible }) => {
  const Logo = bank.logo;
  return (
    <li className={`transition-all duration-500 ease-out ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
      <button
        onClick={onSelect}
      ...
      <main className="flex-grow">
        <ul className="space-y-2">
          {filteredBanks.map((bank) => (
            <BankListItem key={bank.id} bank={bank} onSelect={() => onSelectBank(bank)} visible={visibleItems.has(bank.id)} />
          ))}
        </ul>
      </main>
    </div>
