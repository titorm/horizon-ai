...
const BeneficiaryRow: React.FC<{ beneficiary: Beneficiary, onEdit: () => void, onDelete: () => void }> = ({ beneficiary, onEdit, onDelete }) => {
...
const SuccessionScreen: React.FC = () => {
    ...
    const handleSave = () => {
        ...
        if ('id' in editingBeneficiary) {
            setBeneficiaries(beneficiaries.map(b => b.id === (editingBeneficiary as Beneficiary).id ? (editingBeneficiary as Beneficiary) : b));
        } else {
            ...
            <div className="p-4 md:p-8">
                <header className="flex justify-between items-center mb-6">
                    <div>
                        ...
                        <p className="text-base text-on-surface-variant">Manage your beneficiaries and ensure your assets are protected.</p>
                    </div>
                ...
                >
                    <div className="p-6">
                        <form className="space-y-4" onSubmit={(e) => {e.preventDefault(); handleSave()}}>
                            <Input name="name" label="Full Name" value={editingBeneficiary.name || ''} onChange={handleInputChange} required />
                            <Input name="relationship" label="Relationship" value={editingBeneficiary.relationship || ''} onChange={handleInputChange} required />
                            <Input name="allocation" label="Asset Allocation" placeholder="e.g., 50% of investments" value={editingBeneficiary.allocation || ''} onChange={handleInputChange} required />
                        </form>
                ...
                >
                    <div className="p-6">
                        <p className="text-on-surface-variant mb-6">
                            Are you sure you want to remove {beneficiaryToDelete.name} as a beneficiary? This action cannot be undone.
                        </p>
