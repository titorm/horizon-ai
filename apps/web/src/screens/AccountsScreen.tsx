        ...
        <Card className="p-4 flex items-center gap-4">
            ...
            <div className="text-right">
                ...
                <p className="text-xs text-on-surface-variant">
                    {isSyncing ? "Syncing..." : `Last sync: ${account.lastSync}`}
                </p>
            </div>
            <div className="flex items-center gap-1">
                <Button
                    variant="text"
                    className="px-2 h-auto py-2 !rounded-full"
                    aria-label="Refresh"
...
const AccountsScreen: React.FC<{ isLoading: boolean }> = ({ isLoading }) => {
                ...
                prevAccounts.map((acc) =>
                    acc.id === accountId
                        ? {
                              ...
                              status: "Connected",
                              lastSync: "just now",
                              balance: acc.balance * (1 + (Math.random() - 0.5) * 0.1),
                          } // also update balance slightly
                        : acc
                )
            );
    ...
    return (
        <>
            ...
            {accountToDisconnect && (
                <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="Confirm Account Disconnection">
                    <div className="p-6">
                        <p className="text-on-surface-variant mb-6">
                            Are you sure you want to disconnect your {accountToDisconnect.name} account? This will stop
                            syncing new transactions and remove the account from your dashboard. This action cannot be
                            undone.
                        </p>
                        <div className="flex justify-end gap-3">
                            ...
                            <Button className="bg-error hover:opacity-90" onClick={confirmDisconnect}>
                                Disconnect
                            </Button>
                        </div>
                    </div>
